...
use services::{Configuration, NeuralNodes};
use std::env;
...
async fn main() {
    ...
    let cli = Command::new(env!("CARGO_PKG_NAME"))
        ...
        .about("Interface to setup and manage NeuralNodes servers")
...
fn configuration() -> Configuration {
    let auto_approve = env::var("NeuralNodes_AUTO_APPROVE")
...
async fn start_handler(args: &ArgMatches) {
    ...
    let NeuralNodes = NeuralNodes::open(&config).expect("Failed to open NeuralNodes");
    let service = Arc::new(NeuralNodes);
...
async fn start_receiver_server(service: Arc<NeuralNodes>, port: u16) {
...
fn auth_interceptor(request: Request<()>) -> Result<Request<()>, Status> {
    let secret = env::var("NeuralNodes_SECRET").unwrap_or_default();
    if secret.is_empty() {
        return Ok(request);
    }
...
async fn start_coordinator_server(service: Arc<NeuralNodes>, port: u16) {
